...
// 定义可读的图案创建宏
#define PATTERN(a,b,c,d,e,f,g,h) ((uint64_t)a << 56) | \
                                 ((uint64_t)b << 48) | \
                                 ((uint64_t)c << 40) | \
                                 ((uint64_t)d << 32) | \
                                 ((uint64_t)e << 24) | \
...
// 将 uint64_t 转换为 8字节数组
void uint64ToMatrix(uint64_t input, byte output[8]) {
  for (int i = 0; i < 8; i++) {
...
// 将 8字节数组转换为 uint64_t
uint64_t matrixToUint64(byte input[8]) {
  uint64_t result = 0;
  for (int i = 0; i < 8; i++) {
    result |= (uint64_t)input[i] << (8 * i);
  }
  return result;
}
